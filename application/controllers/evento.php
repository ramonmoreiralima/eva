...
class Evento extends CI_Controller {
	...
	function index() { 
		...
		if ($this->session->userdata ( 'logged_in' )) {
			...
			$data ['titulo'] = "Eventos Disponiveis Para Inscrição";
	...
	function listar_inscrito() { 
		// Validação de Session
		if ($this->session->userdata ( 'logged_in' )) {
			
			$this->load->helper ( 'form' );
			$data ['titulo'] = "Meus Eventos";
                        $id = $this->session->userdata ( 'id_face' );
                        $data ['evento'] = $this->model->listar_inscrito ($id);
			//$data ['evento'] = $this->model->listar ();
			$data ['evento1'] = $this->model->listar_recentes ();
	...
	function inscricao($id) {
		...
		$data ['id_face'] = $this->session->userdata('id_face');
		$data ['id_atividade'] = $id;
		$data ['id_evento'] = $this->session->userdata('id_eve');
		$this->participacao_model->inscricao_participacao($data);
		redirect('/atividade/listar_inscrito/'.$this->session->userdata ('id_eve'), 'refresh');
	}
	function desinscricao($id) {
		/* Inscreve usuario em evento */
		$this->load->model('participacao_model');
		$data ['id_face'] = $this->session->userdata('id_face');
		$data ['id_atividade'] = $id;
		$data ['id_evento'] = $this->session->userdata('id_eve');
		$this->participacao_model->desinscricao_participacao($data);
		redirect('/atividade/listar_nao_inscrito/'.$this->session->userdata ('id_eve'), 'refresh');
	}
